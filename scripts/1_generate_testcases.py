...
# Fichier DBML Produit
DBML_PATH = os.getenv("DBML_PATH", "soda/dbml/produit.dbml")
...
def get_dbml_entry_from_manifest(manifest: Dict[str, Any],
                                 project_name: str = "Produit") -> Dict[str, Any]:
    """
...
def is_produit_table(table) -> bool:
    """
    Détermine si une table appartient au schema PRODUIT ou si son nom commence par PRODUIT.
    """
    schema = getattr(table, "schema", None)
    full_name = getattr(table, "full_name", None)

    if full_name:
        if full_name.upper().startswith("PRODUIT."):
            return True

    if schema and schema.upper() == "PRODUIT":
        return True

    if getattr(table, "name", "").upper().startswith("PRODUIT."):
        return True
